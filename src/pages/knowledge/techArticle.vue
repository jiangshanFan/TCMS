<template>
  <div class="techArticle posr">
    ...
    <thesis v-else-if="show===2" @ifChange="showDefault"></thesis>
            ...
            <template slot-scope="scope">
              ...
              <span v-else-if="h.isShow===1">
                <a
                  ...
                  @click="tothesis(scope.row)"
                >{{scope.row.dissertationCode}}</a>
              </span>
...
/** 导入api.js */
import {
  getDissertationInformation,
  removeDissertationInformation
} from "../../axios/api.js";
import TechArticleAddOrEdit from "../../components/TechArticleAddOrEdit";
import breadcrumbList from "../../components/breadcrumbList";
import thesis from "../../components/teach/thesis.vue";
export default {
  name: "techArticle",
  components: {
  ...
  methods: {
    //点击跳转论文详情
    tothesis(scrow) {
      if (this.$route.meta.button.buttons.includes("技术论文详情")) {
        this.$store.dispatch("knowledge_techArticle", scrow);
        this.show = 2;
        this.breadcrumb.push({ id: "thesis", name: "论文基本信息" });
      }
    },
    ...
    deletes(row) {
      this.$confirm(
        `此操作将删除"${row.dissertationName}"文件, 是否继续?`,
        "提示",
        {
  ...
  data() {
    return {
      ...
      header: [
        { prop: "dissertationCode", label: "论文编号", isShow: 1 },
