<template>
  <div class="patentFollow posr">
    ...
    <knowledge v-else-if="show===2" @ifChange="showDefault"></knowledge>
      ...
      <div class="mt20">
        ...
        >
          ...
          <el-table-column
            ...
            :key="h.prop"
            :label="h.label"
            :width="i !== header.length-1 ? h.label.length*26 : ''"
            align="center"
          ...
          >
            <template slot-scope="scope">
              ...
              <span
                v-else-if="h.multiProp"
              >{{h.multiProp.map(function (item) { if(scope.row[item]) return scope.row[item];}).filter(current => {return current !== null && current !== undefined;}).join('、')}}</span>
              <!--<span v-for="(p,ind) in h.multiProp" :key="ind">{{scope.row[p]}}</span>-->
              <span
                v-else-if="h.date === 1"
              >{{scope.row[h.prop]? $format(new Date(scope.row[h.prop]).getTime()).dates : ''}}</span>
              <span v-else-if="h.isShow===1">
                <a
                  href="javascript:0"
                  style="font-weight: bold; color:rgb(44,140,240);"
                  @click="toTable(scope.row)"
                >{{scope.row.patentCode}}</a>
              </span>
              <!-- 获取方式 -->
              <span v-else-if="h.get===1"></span>
    ...
    <!-- 专利权人变更弹窗 -->
    <el-dialog title="专利权人变更" :visible.sync="dialogVisible" width="60%">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="变更后专利权人：">
              <el-select v-model="form.newPatentee" placeholder="请选择" style="width:100%;">
                <el-option
                  v-for="(item,index) in patentee"
                  :key="index"
                  :label="item.label"
                  :value="item.label"
                ></el-option>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="OA单号：">
              <el-input @blur="oaDanChange" maxlength="15" v-model="form.changeOaOddNum"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="专利信息摘要：">
              <el-input
                maxlength="300"
...
// 引入待授权专利
import patentAuthorize from "../knowledge/patentAuthorize";
export default {
  name: "patentFollow",
  components: {
    ...
    knowledge: knowledge,
    // 引入待授权专利
    patentAuthorize: patentAuthorize
  },
  created() {
    this.getList();
  },
  ...
  methods: {
    ...
    //     this.$money(this.form.changeCost);
    //   } else {
    //     return;
    //   }
    // },
    // oa单号验证接口
    async oaDanChange() {
      if (this.form.changeOaOddNum !== "") {
        let res = await queryOfficeAutomationInformation({
          mathCode: this.form.changeOaOddNum
        });
      }
    },
    // 获取每行专利id
    ...
    async subform() {
      ...
      for (let index of Object.keys(params)) {
        if ("changeCost".match(index)) {
          if (params[index]) {
            params[index] = parseFloat(params[index]) * 100;
          }
        }
      }
      let res = await insertPatentChangeInfo({
        changeConfirmTime: params.changeConfirmTime,
        changeCost: params.changeCost,
        changeOaOddNum: params.changeOaOddNum,
        changeRemarks: params.changeRemarks,
        newPatentee: params.newPatentee,
        pid: this.pid,
        oldPatentee: this.changePeo
      });
      if (res.status === 1) {
        this.$message({
          type: "success",
    ...
    //点击专利编号跳转
    toTable(scrow) {
      if (this.$route.meta.button.buttons.includes("专利详情")) {
      this.$store.dispatch("knowledge_patentFollow", scrow);
      this.show = 2;
      this.breadcrumb.push({ id: "knowledge", name: "专利详情" });
      }
    },
    ...
    async getList() {
      ...
      if (this.search.value1) {
        params.patentApplyNum = this.search.value1;
      }
