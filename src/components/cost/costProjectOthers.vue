<template>
  <div class="costProjectOthers posr">
    <div class="costProjectOtherss pl20 pr20">
          ...
          <el-table-column fixed="right" label="操作" width="120" align="center">
            <template slot-scope="scope">
              <span>
                <el-button
                  class="underline f12"
                  @click="opencostOtherDialog(scope.row)"
                  type="text"
                  align="center"
      ...
      <!-- 新增或编辑其他类别 -->
      <el-dialog
        :title="Object.keys(costOther).length !== 1?'编辑':'新增'"
        :visible.sync="costOtherDialogShow"
        center
            ...
            <el-col :span="24">
              <el-col :span="12">
                <el-form-item label="费用报销单号：" label-width="120px">
                  <el-input @blur="oddCheck" type="text" v-model="costOther.oddNumbers"></el-input>
                </el-form-item>
              </el-col>
            </el-col>
  ...
  methods: {
    async oddCheck() {
      let res = await queryOfficeAutomationInformation({
        mathCode: this.costOther.oddNumbers
      });
      if (res.status === 0) {
        this.costOther.oddNumbers = "";
      }
    },
    ...
    deletes(row) {
      this.$confirm(`此操作将删除"${row.themeTitle}", 是否继续?`, "提示", {
