<template>
  <div class="costProjectMaterial posr">
    <div class="costProjectMaterials pl20 pr20">
      ...
      <!-- 新增或修改材料 -->
      <el-dialog
        :title="Object.keys(costMaterial).length !== 1?'编辑':'新增'"
        :visible.sync="costMaterialDialogShow"
        center
            ...
            <el-col :span="24">
              ...
              <el-col :span="12">
                <el-form-item label="材料名称：" label-width="100px">
                  <el-select
                    v-model="costMaterial.tradeName"
                    filterable
                    remote
                    reserve-keyword
                    placeholder="请输入名称查询材料并添加"
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <b>{{ item.partNo}}</b>
                        <b class="cc">采购数量：</b>
                        <b>{{ item.purchaseNum}}</b>
                        <b class="cc">规格型号：</b>
                        <b>{{ item.specification}}</b>
                        <b class="cc">采购商家：</b>
                        <b>{{ item.purchaseAme}}</b>
...
export default {
  ...
  methods: {
    ...
    // 编辑方法
    async editFun() {
      let res = await editMaterialCostInformation({
        ...this.imp,
        projectId: this.info.projectId,
        fundId: this.info.id
      });
