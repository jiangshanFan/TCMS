<template>
  <div class="costProjectEquipment posr">
    ...
    <div class="costProjectEquipments pl20 pr20">
        ...
        <el-row>
          ...
          <div class="fl mr20 mb20">
            ...
            <el-select v-model="search.value2" placeholder="请选择" size="mini" style="width:calc(100% - 100px);" clearable>
              <el-option v-for="item in options.deviceClass" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
          </div>
      ...
      <div class="mt20">
        <!-- 添加 -->
        <el-button type="primary" size="mini" @click="opencostEquipmentDialog('')" v-if="$route.meta.button.nextButton.includes('新增')">新增</el-button>
        ...
        <el-table
          ...
          <el-table-column fixed="right" label="操作" width="120" align="center">
            <template slot-scope="scope">
              <span>
                <el-button class="underline f12" @click="opencostEquipmentDialog(scope.row)" type="text" align="center" v-if="$route.meta.button.nextButton.includes('编辑')">编辑</el-button>
      ...
      <!-- 新增或修改设备 -->
      <el-dialog :title="Object.keys(costEquipment).length !== 3?'编辑':'新增'" :visible.sync="costEquipmentDialogShow" center custom-class="dialogStyle" @close="cancelAdd">
          ...
          <el-row style="width:100%;line-height:40px;margin:0 auto;">
            ...
            <el-col :span="7">
              <el-form-item label="费用类型：" label-width="100px">
                <el-select v-model="costEquipment.fundType" placeholder="请选择" size="mini" style="width:100%;">
                  <el-option v-for="item in options.fundType" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="9">
              <el-form-item label="设备类别：" label-width="100px">
                <el-select v-model="costEquipment.deviceClass" placeholder="请选择" size="mini" style="width:100%;">
                  <el-option v-for="item in options.deviceClass" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="8">
              <el-form-item label="发票是否盖章：" label-width="120px">
                <el-select v-model="costEquipment.whetherSeal" placeholder="请选择" size="mini" style="width:100%;">
                  <el-option v-for="item in options.whetherSeal" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="24">
              ...
              <el-col :span="12">
                <el-form-item label="设备名称：" label-width="100px">
                  <el-select v-model="costEquipment.equipmentName" filterable remote reserve-keyword placeholder="请输入名称查询设备并添加"
            ...
            <el-col :span="24">
              <el-form-item label="备注：" label-width="100px">
                <el-input type="textarea" :rows="3" resize="none" placeholder="请输入备注内容" v-model="costEquipment.ramark"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
  ...
  export default {
    ...
    methods: {
      ...
      async addOrEditcostEquipment() {
        if (this.costEquipment.deviceClass !== '' && this.costEquipment.fundType !== '' && this.costEquipment.whetherSeal !== '') {
          let params = {
            ...this.costEquipment,
            projectId: this.info.projectId,
            fundId: this.info.id,
    ...
    data() {
      return {
        ...
        header: [
          ...
          { prop: 'deviceClass', label: '设备类别', select: [ { id: 0, label: '项目新购设备'}, { id: 1, label: '预算调配设备'},], eachWidth: 40,},
          { prop: 'whetherSeal', label: '发票是否已盖章', select: [ { id: 0, label: '是'}, { id: 1, label: '否'},], eachWidth: 15,},
