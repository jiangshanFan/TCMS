<template>
  <div class="costProjectOthers posr">
    <div class="costProjectOtherss pl20 pr20">
      ...
      <div class="section-search mb20 mt20">
        <el-row>
          <div class="fl mr20 mb20 lh28">
            <span>
              归属部门：
              <b>{{info.attributionDepartmentName}}</b>
            </span>
          </div>
          ...
          <div class="fl mr20 mb20">
            ...
            >
              <el-option
                v-for="item in options.inOutType"
                :key="item.id"
                :label="item.name"
                :value="item.id"
              ></el-option>
          ...
          <el-table-column fixed="right" label="操作" width="120" align="center">
            <template slot-scope="scope">
              <span>
                <el-button
                  class="underline f12"
                  @click="openmaterialListDetailDialog(scope.row)"
                  type="text"
                  align="center"
      ...
      <!-- 新增经费或预算设置 -->
      <el-dialog
        :title="Object.keys(materialListDetail).length===2?'新增':'编辑'"
        :visible.sync="materialListDetailDialogShow"
        center
      ...
      >
          ...
          <el-row style="width:100%;line-height:60px;margin:0 auto;">
            ...
            <el-col :span="12">
              <el-form-item label="出入库方式：" label-width="120px">
                <el-select
                  @change="projectNameC"
                  v-model="materialListDetail.inOutType"
                  placeholder="请选择"
                  size="mini"
                  style="width:100%;"
                >
                  <el-option
                    v-for="item in options.inOutTypeOne"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id"
                  ></el-option>
            ...
            <el-col :span="12" v-if="materialListDetail.inOutType === 2">
              <el-form-item label="项目名称：" label-width="120px" prop="projectName">
                <el-select
                  v-model="materialListDetail.projectName"
                  placeholder="请选择"
                  size="mini"
                  style="width:100%;"
                >
            ...
            <el-col :span="12" v-else>
              <el-form-item
                :label="materialListDetail.inOutType === 3? '借出部门：': '归还部门：'"
                label-width="120px"
                prop="inOutDepartment"
              >
                <el-select
                  v-model="materialListDetail.inOutDepartment"
                  placeholder="请选择"
                  size="mini"
                  style="width:100%;"
                >
                  <el-option
                    v-for="item in options.inOutDepartment"
                    :key="item.id"
                    :label="item.label"
                    :value="item.id"
                  ></el-option>
            ...
            <el-col :span="24">
              <el-form-item label="备注：" label-width="120px">
                <el-input
                  ...
                  resize="none"
                  placeholder="请输入备注内容"
                  v-model="materialListDetail.remark"
                  maxlength="30"
                  show-word-limit
                ></el-input>
              </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" size="mini" @click="addOrEditmaterialListDetail('formName')">确 定</el-button>
  ...
  methods: {
    ...
    getbranchList() {
      this.header.forEach(async item => {
        if (item.prop === "inOutDepartmentName") {
          let res = await queryDepartmentMaterialManage();
          if (res.status === 1) {
            res.msg.forEach(item => {
              this.options.inOutDepartment.push({
    ...
    // download Excel
    async exportExcel() {
      let res = await downloadProjectProgress({
        projectId: this.search.value1
      });
    },

    // submit materialListDetail
    addOrEditmaterialListDetail(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          if (
            this.materialListDetail.inOutType &&
            this.materialListDetail.intOutNum
          ) {
            let params = {
              ...this.materialListDetail,
              attributionDepartment: this.info.attributionDepartment,
  ...
  data() {
    return {
      ...
      header: [
        ...
        { prop: "inOutDepartmentName", label: "借出/归还部门" },
