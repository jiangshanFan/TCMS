<template>
  <div class="UserNameAddOrEdit p20">
    <el-row>
      <el-col style="margin-left:18%;"></el-col>
    </el-row>
    <el-row>
      <el-col :span="12" :offset="6">
        <el-form
          ...
          status-icon
          :rules="rules"
          ref="UserNameAddOrEdit"
          label-width="130px"
          label-position="right"
          class="mt20"
          size="small"
        >
          <el-form-item label="新增账户说明：">
            <span style="font-size:14px; color:red;">登录用户无角色管理查看权限则无法设置“用户角色”.</span>
          </el-form-item>
          ...
          <el-form-item
            label="工号："
            prop="workNo"
            :rules="[{ required: true, max: 10, message: '不能超过十个数字！', trigger: ['blur', 'change'] }]"
          >
            <!-- v-if="form.role.id !== 5"-->
            <el-input
              v-model="form.workNo"
          ...
          <el-form-item
            label="手机号："
            prop="mobile"
            :rules="[{ required: true, min:11, max: 11, message: '请输入11位手机号码', trigger: 'change' }]"
          >
...
export default {
  ...
  async created() {
    // get deptList
    let res1 = await queryDepartmentListManage();
    if (res1.status === 1) {
  ...
  data() {
    let validateAccount = (rule, value, callback) => {
      if (!value) {
        ...
        callback(
          new Error("请正确填写，支持英文大小写、数字和下划线_，20字以内")
        );
      }
    };
    return {
